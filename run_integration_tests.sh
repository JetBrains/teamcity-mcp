#!/usr/bin/env bash
# Run MCP integration tests against a TeamCity server.
#
...
#   TC_DIST=path/to/TeamCity-*.tar.gz ./run_integration_tests.sh
#
# In mode 2, the plugin is built first (unless PLUGIN_ZIP is set),
# a fresh TC server is started, tests run, and the server is stopped.
