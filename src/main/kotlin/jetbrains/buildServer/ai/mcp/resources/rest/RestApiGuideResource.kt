...
@Component
class RestApiGuideResource : BraveModeAwareMcpResource {
    ...
    override val description =
        "Compact operating guide for AI agents using teamcity_rest_get, teamcity_rest_post, and teamcity_build_log. " +
        "Covers endpoint discovery, locators, field selection, pagination, safe personal-build triggering, log navigation, and common TeamCity investigation workflows."
    ...
    **Common mistakes:**
    ...
    | `query` | No | Extra query parameters without leading `?` (for example `moveToTop=true`) |
    ...
    **Step 4** — If the build fails, investigate:
    ...
    - **Use `query` for endpoint flags** such as `moveToTop=true`; use `fields` for response shaping.
